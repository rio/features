...
main () {
    ...
    if [ "${VCLUSTER_VERSION}" != "latest" ] ; then
        VCLUSTER_CHECKSUM_URL="https://github.com/loft-sh/vcluster/releases/download/v${VCLUSTER_VERSION#[vV]}/vcluster-linux-${ARCH}.sha256"
        ...
        VCLUSTER_CHECKSUM_URL="$(echo "${RELEASES_RESPONSE}" | grep "browser_download_url.*vcluster-linux-${ARCH}.sha256" | head -n 1 | cut -d '"' -f 4)"
    ...
    wget --no-verbose -O /tmp/checksum.sha256 "${VCLUSTER_CHECKSUM_URL}"

    echo "Downloading ${VCLUSTER_URL} ..."
    wget --no-verbose -O /tmp/vcluster "${VCLUSTER_URL}"

    echo "Verifying checksum $(cat /tmp/checksum.sha256) ..."
    echo "$(cat /tmp/checksum.sha256)  /tmp/vcluster" | sha256sum -c -

    echo "Installing..."
    mv /tmp/vcluster /usr/local/bin/vcluster
    chmod +x /usr/local/bin/vcluster
    rm /tmp/checksum.sha256
